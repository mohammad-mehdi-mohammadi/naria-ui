...
export const PaginationPrev = ({ children, classNames }: { children?: ReactNode, classNames?: { prev?: string } }) => {
...
export const PaginationNext = ({ children, classNames }: { children?: ReactNode, classNames?: { next?: string } }) => {
    const context = useContext(PaginationContext);
    if (!context) throw new Error("Pagination components must be used within Pagination");
...
export const PaginationPages = ({ separator, classNames }: {
    separator?: ReactNode,
    classNames?: { page?: string; active?: string; separator?: string; pagesContainer?: string }
}) => {
    const context = useContext(PaginationContext);
    if (!context) throw new Error("Pagination components must be used within Pagination");
    const { page, totalPages, onPageChange } = context;
    ...
    const getPageNumbers = () => {
        ...
        if (totalPages <= maxVisible + 2) {
            ...
            if (page <= 2) {
                ...
                left = totalPages - 2; right = totalPages - 1;
            } else {
        ...
        <div className={`naria-pagination__pages-container ${classNames?.pagesContainer || ""}`}>
            {pageNumbers.map((p, idx) =>
                typeof p === "number" ? (
                    ...
                    <span
                        key={`separator-${idx}`}
                        className={`naria-pagination__separator ${classNames?.separator || ""}`}
...
export const PaginationContent = ({ children, classNames }: { children?: ReactNode | ((context: { page: number; totalPages: number; onPageChange: (v: number) => void }) => ReactNode), classNames?: { content?: string } }) => {
    const context = useContext(PaginationContext);
    if (!context) throw new Error("Pagination components must be used within Pagination");
...
const PaginationBase: FC<Props> = ({ page, size, total, onPageChange, children, classNames }) => {
    const totalPages = Math.ceil(total / size);
    const value = { page, totalPages, onPageChange };
...
export const Pagination = PaginationBase as PaginationComponent;
