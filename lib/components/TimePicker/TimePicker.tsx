...
export interface TimePickerProps {
    ...
    labels?: {
        am?: ReactNode | string;
        pm?: ReactNode | string;
        toggleLabel?: ReactNode | string | undefined;
    };
    enabledParts?: {
        hour?: boolean;
        minute?: boolean;
...
export const TimePicker: FC<TimePickerProps> = ({
        ...
        return (
            <div className={`naria-time-picker ${classNames?.root || ''} ${disabled ? 'naria-time-picker--disabled' : ''}`} data-class-prop="root">
                <div className={`naria-time-picker__wrapper ${classNames?.wrapper || ''}`} data-class-prop="wrapper">
                    <div className={`naria-time-picker__controls ${classNames?.controls || ''}`} data-class-prop="controls">
                        {
                            enabledParts.hour && mode === '12' ? (
                                <div className={`naria-time-picker__ampm-wrapper ${classNames?.ampmWrapper || ''}`} data-class-prop="ampmWrapper">
                                    <button type="button" onClick={() => handleAmPm('am')}
                                            className={`naria-time-picker__am ${classNames?.am || ''}`} data-class-prop="am">
                                        {labels.am}
                                    </button>
                                    <button type="button" onClick={() => handleAmPm('pm')}
                                            className={`naria-time-picker__pm ${classNames?.pm || ''}`} data-class-prop="pm">
                                        {labels.pm}
                                    </button>
                                </div>
                            ) : undefined
                        }
                        <div className={`naria-time-picker__toggle-wrapper ${classNames?.toggleWrapper || ''}`} data-class-prop="toggleWrapper">
                            {activeButtons.map((btn, index) => (
                                <React.Fragment key={btn.mode}>
                                    ...
                                    >
                                        {
                                            isToggleLabelEnable ? <div className={`naria-time-picker__toggle-label ${classNames?.toggleLabel || ''}`} data-class-prop="toggleLabel">{labels?.toggleLabel ? labels.toggleLabel : btn.label}</div> : undefined
                                        }
                                        <div className={`naria-time-picker__toggle-value ${classNames?.toggleValue || ''}`} data-class-prop="toggleValue">
                                    ...
                                    {index < activeButtons.length - 1 && (
                                        <span className={`naria-time-picker__time-separator ${classNames?.timeSeparator || ''}`} data-class-prop="timeSeparator">:</span>
                                    )}
                                </React.Fragment>
                            ))}
                        </div>
                    ...
                    >
                        ...
                        {timeMode === "hour" && enabledParts.hour && mode === "24" && (
                            <div className={`naria-time-picker__inner-ring ${classNames?.innerRing || ''}`} data-class-prop="innerRing">
                        ...
                        {(timeMode === "minute" || timeMode === "second") && (
                            <div className={`naria-time-picker__minute-markers ${classNames?.minuteMarkers || ''}`} data-class-prop="minuteMarkers">
                                {minutes.map((unit) => {
                                    ...
                                    return (
                                        <div
                                            key={unit}
                                            className={`naria-time-picker__minute-dot ${classNames?.minuteDot || ''}`}
                                            style={{left: `${x}%`, top: `${y}%`}}
                        ...
                        {(timeMode === "minute" || timeMode === "second") && (
                            <div className={`naria-time-picker__minute-number-wrapper ${classNames?.minuteNumberWrapper || ''}`} data-class-prop="minuteNumberWrapper">
                        ...
                        <div className={`naria-time-picker__center-dot ${classNames?.centerDot || ''}`} data-class-prop="centerDot"/>
                    </div>
                </div>
            </div>
        );
